...
- a generated local builder-image Containerfile
...
The default image is built locally as
`local/hexbox-builder:alpine-3.22-lix-2.95.2-1` from an Alpine 3.22 + Lix
Containerfile generated by the module.
...
- Building the local builder image requires access to Alpine package mirrors and
  the Lix installer when the image is missing.
